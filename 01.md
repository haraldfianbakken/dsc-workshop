...
We'll go ahead and add some more examples to the configuration. Now go ahead and add a Windows feature to install IIS. Also, create a script resource to check if a file exists at a path. If not, create it with some content. 
Creating a file with contents can also be done with the "File" resource. Do this as a seperate exercise. 

The script block can be very useful as it's highly flexible. 
